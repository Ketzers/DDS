"""
dashboard.py — главная страница СППР: лендинг с навигацией.

Запуск (из корня проекта):
    streamlit run dashboard.py

Структура приложения:
    dashboard.py                       ← этот файл (лендинг с навигацией)
    pages/00_Explore_market.py         ← карта, аналитика, кластеры
    pages/01_Evaluate_project.py       ← форма проекта + оценка + рекомендации
    pages/02_Compare_projects.py       ← сравнение нескольких проектов
    sppr/                              ← скоринговый движок
    .streamlit/config.toml             ← светлая тема

Зависимости:
    pip install streamlit pandas folium streamlit-folium plotly scipy openpyxl scikit-learn
"""
...
# ── Путь к корню (нужен для загрузки данных на лендинге) ────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# ЗАГРУЗКА МЕТРИК ДЛЯ БЛОКА «О ДАННЫХ»
# ─────────────────────────────────────────────────────────────────────────────
...
# Проверяем наличие st.switch_page (Streamlit 1.36+)
HAS_SWITCH_PAGE = hasattr(st, 'switch_page')

col1, col2, col3 = st.columns(3)

# ── Карточка 1: Изучить рынок ───────────────────────────────────────────
with col1:
    st.markdown("""
...
# ─────────────────────────────────────────────────────────────────────────────
# ФУТЕР: МЕТОДОЛОГИЯ И ОГРАНИЧЕНИЯ
# ─────────────────────────────────────────────────────────────────────────────
st.markdown("")
st.markdown("---")
